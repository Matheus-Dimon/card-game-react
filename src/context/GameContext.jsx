...
function safeLaneCopy(field = {}) {
  return { melee: Array.isArray(field.melee) ? [...field.melee] : [], ranged: Array.isArray(field.ranged) ? [...field.ranged] : [] }
}
...
function applyHeroPowerEffect(state, playerKey, power) {
  ...
  if (power.type==='DAMAGE') {
    ...
    return {...state, [playerKey]:{...player, mana:player.mana - power.cost, hasUsedHeroPower:true}, [opponentKey]:opponent}
  }
  return {...state, [playerKey]:{...player, mana:player.mana - power.cost, hasUsedHeroPower:true}}
}
...
function reducer(state=initialState, action){
  switch(action.type){
    ...
    case 'GO_TO_HERO_POWER_OPTIONS': return {...state, gamePhase:'HERO_POWER_OPTIONS'}
    ...
    case 'START_GAME': {
      ...
      const draw=(p,n)=>{
        ...
        p.hand=[...p.hand,...drawn]; p.deck=p.deck.slice(n)
      }
      draw(p1,3)
      draw(p2,3)
    ...
    case 'INITIATE_ATTACK_ANIMATION': {
      ...
      return {...state, animation:{
        ...
        startRect:null,
        endRect:null,
        callbackAction:{type:'APPLY_ATTACK_DAMAGE', payload:{attackerId,targetId,targetIsHero,damage,playerKey}}
      }}
    }

    case 'END_ANIMATION': {
      const {callbackAction}=state.animation
      let newState={...state, animation:{active:false, element:null, startRect:null, endRect:null, callbackAction:null}}
      if(callbackAction) newState=reducer(newState, callbackAction)
      return newState
    }
    ...
    const runAI=async()=>{
      ...
      for(const attacker of attackers){
        ...
        if(enemyField.length>0){
          ...
          dispatch({type:'INITIATE_ATTACK_ANIMATION',payload:{attackerId:attacker.id,targetId:target.id,targetIsHero:false,damage:attacker.attack||1,playerKey:'player2'}})
        } else {
          dispatch({type:'INITIATE_ATTACK_ANIMATION',payload:{attackerId:attacker.id,targetId:null,targetIsHero:true,damage:attacker.attack||1,playerKey:'player2'}})
        }
        await delay(500)
      }
